    ...
    >
      <div className="flex flex-wrap gap-2">
        {customRound ? (
          ...
          <Select
            ...
            className="max-w-[9rem]"
            onChange={(event) => {
              if (event.target.value === "__nova__") onCustomRound();
            }}
          >
            {rounds.map((round) => (
        ...
        <GameDateField defaultValue={toDatetimeLocal(game.date)} className="max-w-[12rem]" />
      </div>
      <div className="flex flex-wrap items-center gap-2">
        <Select name="team_a_id" defaultValue={game.team_a_id} className="max-w-[10rem]">
          {teams.map((team) => (
        ...
        <Select name="team_b_id" defaultValue={game.team_b_id} className="max-w-[10rem]">
          {teams.map((team) => (
            <option key={team.id} value={team.id}>
              {team.name}
            </option>
        ...
        {venues.length > 0 && (
          <Select name="venue_id" defaultValue={game.venue_id ?? ""} className="max-w-[10rem]">
        ...
        {referees.length > 0 && (
          <Select name="referee_id" defaultValue={game.referee_id ?? ""} className="max-w-[10rem]">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <label className="flex items-center gap-2 text-xs text-muted">
          <input type="checkbox" name="referee_paid" defaultChecked={game.referee_paid} />
          Árbitro pago
        </label>
      </div>
      <p className="text-xs text-muted">Placar e &quot;jogo realizado&quot; ficam na aba Súmula.</p>
...
function GameActions({
  ...
  return (
    <div className="flex flex-wrap justify-end gap-2">
      <Button variant="secondary" onClick={onToggleSumula}>
        {sumulaOpen ? "Fechar súmula" : "Súmula"}
      </Button>
...
export function GameTable({
  ...
  async function handleDeleteGame(game: Game) {
    const ok = await confirm({
      title: `Excluir o jogo "${teamName(game.team_a_id)} x ${teamName(game.team_b_id)}"?`,
                      ...
                      {editingId === game.id ? (
                        <td colSpan={8} className="px-4 py-4">
                          <GameEditForm
                            ...
                            onCustomRound={() => setCustomRound(true)}
                            onDone={stopEdit}
                            onCancel={stopEdit}
                          />
                        </td>
                        ...
                        <>
                          ...
                          <td className="px-4 py-3">
                            <GameActions
                              sumulaOpen={eventingId === game.id}
                              onToggleSumula={() =>
                                setEventingId(eventingId === game.id ? null : game.id)
                              }
                              onEdit={() => startEdit(game.id)}
        ...
        <div className="flex flex-col gap-3 sm:hidden">
          {filteredGames.map((game) => (
            <Fragment key={game.id}>
                ...
                {editingId === game.id ? (
                  ...
                  <>
                    ...
                    <dl className="mt-2 space-y-1 text-xs text-muted">
                      ...
                      {refereeName(game.referee_id) && (
                        <div className="flex items-center gap-2">
                          Árbitro: {refereeName(game.referee_id)}
                          {game.referee_payment_amount !== null && (
                            <Badge tone={game.referee_paid ? "success" : "warning"}>
                              {game.referee_paid ? "Pago" : "A pagar"} · R${" "}
                              {game.referee_payment_amount.toFixed(2)}
                    ...
                    <div className="mt-3">
                      <GameActions
                        sumulaOpen={eventingId === game.id}
                        onToggleSumula={() => setEventingId(eventingId === game.id ? null : game.id)}
                        onEdit={() => startEdit(game.id)}
                        onDelete={() => handleDeleteGame(game)}
                      />
                    </div>
              ...
              {eventingId === game.id && <Card className="p-3">{renderSumula(game)}</Card>}
            </Fragment>
          ))}
        </div>
      </>
